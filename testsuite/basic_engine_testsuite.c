...
static enum test_result allocate_test(ENGINE_HANDLE *h, ENGINE_HANDLE_V1 *h1) {
    item *test_item = NULL;
    void *key = "akey";
    assert(h1->allocate(h, NULL, &test_item, key, strlen(key), 1,1,1,
...
static enum test_result expiry_test(ENGINE_HANDLE *h, ENGINE_HANDLE_V1 *h1) {
    ...
    void *key = "get_test_key";
    uint64_t cas = 0;
    assert(h1->allocate(h, NULL, &test_item, key, strlen(key), 1, 0, 10,
                        PROTOCOL_BINARY_RAW_BYTES) == ENGINE_SUCCESS);
    assert(h1->store(h, NULL, test_item, &cas, OPERATION_SET, 0) == ENGINE_SUCCESS);
...
#define max_threads 30
    ...
    for (ii = 0; ii < max_threads; ++ii) {
        assert(cb_create_thread(&tid[ii], incr_test_main, h, 0) == 0);
    }

    for (ii = 0; ii < max_threads; ++ii) {
        assert(cb_join_thread(tid[ii]) == 0);
    }
...
static enum test_result get_item_info_test(ENGINE_HANDLE *h, ENGINE_HANDLE_V1 *h1) {
    ...
    assert(h1->allocate(h, NULL, &test_item, key, strlen(key), 1,0, exp,
                        PROTOCOL_BINARY_RAW_BYTES) == ENGINE_SUCCESS);
    assert(h1->store(h, NULL, test_item, &cas, OPERATION_SET,0) == ENGINE_SUCCESS);
...
static enum test_result touch_test(ENGINE_HANDLE *h, ENGINE_HANDLE_V1 *h1) {
    ...
    r.touch.message.header.request.keylen = htons((uint16_t)keylen);
